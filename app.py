...
# model.pyの中から predictのみをインポートして使用する。
from model import predict

st.set_option("deprecation.showfileUploaderEncoding", False)

st.sidebar.title("画像認識アプリ")
st.sidebar.write("Pytorch ResNetを使って何の画像かを判定します。")

st.sidebar.write("") # スペース1行
...
if img_source == "画像をアップロード":
    ...
    img_file = st.camera_input("カメラで撮影") # カメラモジュールは公式ドキュメントから拝借。

# 画像がある場合推定する。
if img_file is not None:
    with st.spinner("推定中..."):
        img = Image.open(img_file).convert("RGB")  # ここでRGBに変換
